...
sjcl.cipher.aes = function (a) {
  this.o[0][0][0] || this.r();
  var b, c, d, e, f = this.o[0][4], g = this.o[1];
  b = a.length;
  var h = 1;
...
sjcl.cipher.aes.prototype = {encrypt: function (a) {
  ...
  var a = this.o[0], b = this.o[1], c = a[4], d = b[4], e, f, g, h = [], i = [], k, j, l, m;
  ...
  var c = this.d[b], d = a[0] ^ c[0], e = a[b ? 3 : 1] ^ c[1], f = a[2] ^ c[2];
  a = a[b ? 1 : 3] ^ c[3];
...
sjcl.codec.base64 = {K: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", fromBits: function (a, b, c) {
  var d = "", e = 0, f = sjcl.codec.base64.K, g = 0, h = sjcl.bitArray.bitLength(a);
  ...
  a = a.replace(/\s|=/g, "");
  var c = [], d = 0, e = sjcl.codec.base64.K, f = 0, g;
  if (b) e = e.substr(0, 62) + "-_";
  for (b = 0; b < a.length; b++) {
    g = e.indexOf(a.charAt(b));
    if (g < 0) throw new sjcl.exception.invalid("this isn't base64: '" + a.charAt(b) + "', " + b + " -- " + a.length);
    if (d > 26) {
      d -= 26;
      c.push(f ^ g >>> d);
      f = g << 32 - d;
...
sjcl.hash.sha256.prototype = {blockSize: 512, reset: function () {
  ...
  a: for (; b < 64; c++) {
    ...
    if (b < 8) this.l[b] = (Math.pow(c, 0.5) - Math.floor(Math.pow(c, 0.5))) * 4294967296 | 0;
    this.d[b] = (Math.pow(c, 0.3333333333333333) - Math.floor(Math.pow(c, 0.3333333333333333))) * 4294967296 | 0;
    b++;
  }
}, g: function (a) {
  var b, c, d = a.slice(0), e = this.c, f = this.d, g = e[0], h = e[1], i = e[2], k = e[3], j = e[4], l = e[5], m = e[6], n = e[7];
  for (a = 0; a < 64; a++) {
    if (a < 16) b = d[a]; else {
      b = d[a + 1 & 15];
      c = d[a + 14 & 15];
...
sjcl.mode.ccm = {name: "ccm", encrypt: function (a, b, c, d, e) {
  ...
  if (b < 15 - g) b = 15 - g;
  c = f.clamp(c, 8 * (15 - b));
  i = sjcl.mode.ccm.O(a, i, c, k, e, b);
  a = sjcl.mode.ccm.M(a, i.data, c, d, e, b);
  ...
  if (d.length) {
    c = h.bitLength(d) / 8;
    if (c <= 65279) g = [h.partial(16, c)]; else if (c <= 4294967295) g = h.concat([h.partial(16, 65534)], [c]);
    g = h.concat(g, d);
...
sjcl.mode.ocb2 = {name: "ocb2", encrypt: function (a, b, c, d, e, f) {
  ...
  var g, h = sjcl.mode.ocb2.H, i = sjcl.bitArray, k = i.k, j = [0, 0, 0, 0];
  c = h(a.encrypt(c));
  var l, m = [];
  d = d || [];
  e = e || 64;
  ...
  if (sjcl.bitArray.bitLength(c) !== 128) throw new sjcl.exception.invalid("ocb iv must be 128 bits");
  e = e || 64;
  var g = sjcl.mode.ocb2.H, h = sjcl.bitArray, i = h.k, k = [0, 0, 0, 0], j = g(a.encrypt(c)), l, m, n = sjcl.bitArray.bitLength(b) - e, o = [];
  d = d || [];
  ...
  var c, d = sjcl.mode.ocb2.H, e = sjcl.bitArray, f = e.k, g = [0, 0, 0, 0], h = a.encrypt([0, 0, 0, 0]);
  h = f(h, d(d(h)));
...
sjcl.misc.hmac.prototype.encrypt = sjcl.misc.hmac.prototype.mac = function (a, b) {
  a = new this.p(this.e[0]).update(a, b).finalize();
  return new this.p(this.e[1]).update(a).finalize();
};
...
sjcl.hash.sha1.prototype = {blockSize: 512, reset: function () {
  ...
  this.reset();
  return c;
}, l: [1732584193, 4023233417, 2562383102, 271733878, 3285377520], d: [1518500249, 1859775393, 2400959708, 3395469782], X: function (a, b, c, d) {
  if (a <= 19) return b & c | ~b & d; else if (a <= 39) return b ^ c ^ d; else if (a <= 59) return b & c | b & d | c & d; else if (a <= 79) return b ^ c ^ d;
}, z: function (a, b) {
  return b << a | b >>> 32 - a;
}, g: function (a) {
  var b, c, d, e, f, g, h = a.slice(0), i = this.c;
  c = i[0];
  d = i[1];
  e = i[2];
  f = i[3];
  g = i[4];
  for (a = 0; a <= 79; a++) {
    if (a >= 16) h[a] = this.z(1, h[a - 3] ^ h[a - 8] ^ h[a - 14] ^ h[a - 16]);
    b = this.z(5, c) + this.X(a, d, e, f) + g + h[a] + this.d[Math.floor(a / 20)] | 0;
    g = f;
    f = e;
    e = this.z(30, d);
    d = c;
...
sjcl.bn.prototype = {radix: 24, maxMul: 8, f: sjcl.bn, copy: function () {
  ...
  if (typeof a === "number") a = new this.f(a);
  var b = 0, c;
  this.fullReduce();
  a.fullReduce();
  ...
  if (typeof a === "number") a = new this.f(a);
  var b = 0, c = 0, d, e, f;
  for (d = Math.max(this.limbs.length, a.limbs.length) - 1; d >= 0; d--) {
    e = this.getLimb(d);
    f = a.getLimb(d);
    c |= f - e & ~b;
    b |= e - f & ~c;
  ...
  for (b = 0; b < this.limbs.length; b++) {
    for (c = d[b].toString(16); b < this.limbs.length - 1 && c.length < 6;) c = "0" + c;
    a = c + a;
  }
  return "0x" + a;
}, addM: function (a) {
  if (typeof a !== "object") a = new this.f(a);
  ...
  for (var b = new sjcl.bn(this).normalize(), c = 0; b.greaterEquals(a); c++) a.doubleM();
  for (; c > 0; c--) {
    a.halveM();
    b.greaterEquals(a) && b.subM(a).normalize();
  }
  return b.trim();
  ...
  if (typeof a === "number") a = new this.f(a);
  var b, c = this.limbs, d = a.limbs, e = c.length, f = d.length, g = new this.f, h = g.limbs, i, k = this.maxMul;
  for (b = 0; b < this.limbs.length + a.limbs.length + 1; b++) h[b] = 0;
  for (b = 0; b < e; b++) {
    i = c[b];
    for (a = 0; a < f; a++) h[b + a] += i * d[a];
    if (!--k) {
      k = this.maxMul;
      g.cnormalize();
    }
  ...
  if (typeof a === "number") a = [a]; else if (a.limbs !== undefined) a = a.normalize().limbs;
  var b, c, d = new this.f(1), e = this;
  for (b = 0; b < a.length; b++) for (c = 0; c < this.radix; c++) {
    if (a[b] & 1 << c) d = d.mul(e);
    e = e.square();
  }
  return d;
}, mulmod: function (a, b) {
  ...
  for (b = 0; b < f || a !== 0 && a !== -1; b++) {
    a = (e[b] || 0) + a;
    d = e[b] = a & g;
    a = (a - d) * c;
  }
  if (a === -1) e[b - 1] -= this.placeVal;
  return this;
}, cnormalize: function () {
  var a = 0, b, c = this.ipv, d, e = this.limbs, f = e.length, g = this.radixMask;
  for (b = 0; b < f - 1; b++) {
  ...
  for (b--; b >= 0; b--) a = c.concat(a, [c.partial(this.radix, this.getLimb(b))]);
  return a;
}, bitLength: function () {
  this.fullReduce();
...
sjcl.bn.pseudoMersennePrime = function (a, b) {
  ...
  d.reduce = function () {
    var g, h, i, k = this.modOffset, j = this.limbs, l = this.offset, m = this.offset.length, n = this.factor, o;
    for (g = this.minOffset; j.length > k;) {
      i = j.pop();
      o = j.length;
      for (h = 0; h < m; h++) j[o + l[h]] -= n[h] * i;
      g--;
      if (!g) {
        j.push(0);
        this.cnormalize();
  ...
  d.U = d.fullMask === -1 ? d.reduce : function () {
    var g = this.limbs, h = g.length - 1, i, k;
    this.reduce();
    if (h === this.modOffset - 1) {
      k = g[h] & this.fullMask;
      g[h] -= k;
      for (i = 0; i < this.fullOffset.length; i++) g[h + this.fullOffset[i]] -= this.fullFactor[i] * k;
      this.normalize();
    }
  };
  d.fullReduce = function () {
    ...
    this.normalize();
    this.U();
    for (h = this.limbs.length; h < this.modOffset; h++) this.limbs[h] = 0;
    g = this.greaterEquals(this.modulus);
    for (h = 0; h < this.limbs.length; h++) this.limbs[h] -= this.modulus.limbs[h] * g;
    this.cnormalize();
    return this;
  };
  d.inverse = function () {
...
sjcl.hash.md5.hash = function (a) {
  return (new sjcl.hash.md5).update(a).finalize();
};
sjcl.hash.md5.prototype = {blockSize: 512, reset: function () {
  this.c = this.l.slice(0);
  this.b = [];
  this.a = 0;
  return this;
  ...
  b = this.a;
  a = this.a = b + sjcl.bitArray.bitLength(a);
  for (b = this.blockSize + b & -this.blockSize; b <= a; b += this.blockSize) this.g(c.splice(0, 16), true);
  return this;
}, finalize: function () {
  var a, b = this.b, c = this.c;
  b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]);
  for (a = b.length + 2; a & 15; a++) b.push(0);
  b.push(this.a | 0);
  for (b.push(this.a / 4294967296 | 0); b.length;) this.g(b.splice(0, 16), b.length);
  this.reset();
  this.I(c, 4);
  return c;
}, l: [1732584193, 4023233417, 2562383102, 271733878], I: function (a, b) {
  var c, d;
  for (c = 0; c < b; c++) {
    d = a[c];
    a[c] = d >>> 24 | d >> 8 & 65280 | (d & 65280) << 8 | (d & 255) << 24;
  }
}, A: [], r: function () {
  var a;
  for (a = 0; a < 64; a++) this.A[a] = 4294967296 * Math.abs(Math.sin(a + 1)) | 0;
}, g: function (a, b) {
  var c, d, e, f = a.slice(0), g = this.c, h = this.A;
  a = g[0];
  c = g[1];
  d = g[2];
  e = g[3];
...
sjcl.misc.hmac_sha1 = function (a, b) {
  this.p = b = b || sjcl.hash.sha1;
  var c = [[], []], d = b.prototype.blockSize / 32;
  this.e = [new b, new b];
  if (a.length > d) a = b.hash(a);
  for (b = 0; b < d; b++) {
...
sjcl.misc.hmac_sha1.prototype.encrypt = sjcl.misc.hmac_sha1.prototype.mac = function (a, b) {
  a = new this.p(this.e[0]).update(a, b).finalize();
  return new this.p(this.e[1]).update(a).finalize();
};
...
sjcl.random = {randomWords: function (a, b) {
  ...
  b = this.isReady(b);
  var d;
  if (b === 0) throw new sjcl.exception.notReady("generator isn't seeded"); else b & 2 && this.$(!(b & 1));
  for (b = 0; b < a; b += 4) {
    (b + 1) % 65536 === 0 && this.R();
    d = this.F();
  ...
  c = c || "user";
  var d, e, f = (new Date).valueOf(), g = this.w[c], h = this.isReady();
  d = this.L[c];
  if (d === undefined) d = this.L[c] = this.W++;
  if (g === undefined) g = this.w[c] = 0;
  this.w[c] = (this.w[c] + 1) % this.h.length;
  switch (typeof a) {
    case "number":
      break;
    case "object":
      ...
      this.h[g].update([d, this.P++, 2, b, f, a.length].concat(a));
      break;
    case "string":
      if (b === undefined) b = a.length;
      this.h[g].update([d, this.P++, 3, b, f, a.length]);
      this.h[g].update(a);
      break;
    default:
  ...
  this.m += b;
  if (h === 0) {
    this.isReady() !== 0 && this.Q("seeded", Math.max(this.n, this.m));
    this.Q("progress", this.getProgress());
  }
}, isReady: function (a) {
  a = this.J[a !== undefined ? a : this.D];
  return this.n && this.n >= a ? this.q[0] > 80 && (new Date).valueOf() > this.S ? 3 : 1 : this.m >= a ? 2 : 0;
}, getProgress: function (a) {
  a = this.J[a ? a : this.D];
  return this.n >= a ? 1["0"] : this.m > a ? 1["0"] : this.m / a;
}, startCollectors: function () {
  if (!this.s) {
    if (window.addEventListener) {
      window.addEventListener("load", this.t, false);
      window.addEventListener("mousemove", this.u, false);
  ...
  if (this.s) {
    if (window.removeEventListener) {
      window.removeEventListener("load", this.t, false);
      window.removeEventListener("mousemove", this.u, false);
...
}, h: [new sjcl.hash.sha256], q: [0], G: 0, w: {}, P: 0, L: {}, W: 0, n: 0, m: 0, S: 0, d: [0, 0, 0, 0, 0, 0, 0, 0], j: [0, 0, 0, 0], C: undefined, D: 6, s: false, B: {progress: {}, seeded: {}}, V: 0, J: [0, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024], F: function () {
  for (var a = 0; a < 4; a++) {
    this.j[a] = this.j[a] + 1 | 0;
    if (this.j[a]) break;
  }
  return this.C.encrypt(this.j);
}, R: function () {
  this.d = this.F().concat(this.F());
  this.C = new sjcl.cipher.aes(this.d);
}, Z: function (a) {
  this.d = sjcl.hash.sha256.hash(this.d.concat(a));
  this.C = new sjcl.cipher.aes(this.d);
  for (a = 0; a < 4; a++) {
    this.j[a] = this.j[a] + 1 | 0;
    if (this.j[a]) break;
  ...
  var b = [], c = 0, d;
  this.S = b[0] = (new Date).valueOf() + 3e4;
  for (d = 0; d < 16; d++) b.push(Math.random() * 4294967296 | 0);
  for (d = 0; d < this.h.length; d++) {
    b = b.concat(this.h[d].finalize());
    c += this.q[d];
    this.q[d] = 0;
    if (!a && this.G & 1 << d) break;
  }
  if (this.G >= 1 << this.h.length) {
    this.h.push(new sjcl.hash.sha256);
    this.q.push(0);
  }
  this.m -= c;
  if (c > this.n) this.n = c;
  this.G++;
  this.Z(b);
}, u: function (a) {
...
sjcl.json = {defaults: {v: 1, iter: 1e3, ks: 128, ts: 64, mode: "ccm", adata: "", cipher: "aes"}, encrypt: function (a, b, c, d) {
  c = c || {};
  d = d || {};
  var e = sjcl.json, f = e.i({iv: sjcl.random.randomWords(4, 0)}, e.defaults);
  e.i(f, c);
  if (typeof f.salt === "string") f.salt = sjcl.codec.base64.toBits(f.salt);
  if (typeof f.iv === "string") f.iv = sjcl.codec.base64.toBits(f.iv);
  if (!sjcl.mode[f.mode] || !sjcl.cipher[f.cipher] || typeof a === "string" && f.iter <= 100 || f.ts !== 64 && f.ts !== 96 && f.ts !== 128 || f.ks !== 128 && f.ks !== 192 && f.ks !== 256 || f.iv.length < 2 || f.iv.length > 4) throw new sjcl.exception.invalid("json encrypt: invalid parameters: " + JSON.stringify(f));
  if (typeof a === "string") {
  ...
  return e.encode(e.aa(f, e.defaults));
}, decrypt: function (a, b, c, d) {
  c = c || {};
  d = d || {};
  var e = sjcl.json;
  b = e.i(e.i(e.i({}, e.defaults), e.decode(b)), c, true);
  ...
  a = a.replace(/\s/g, "");
  if (!a.match(/^\{.*\}$/)) throw new sjcl.exception.invalid("json decode: this isn't json: " + a);
  a = a.replace(/^\{|\}$/g, "").split(/,/);
  var b = {}, c, d;
  for (c = 0; c < a.length; c++) {
...
sjcl.mode.cbc = {name: "cbc", encrypt: function (a, b, c, d) {
  ...
  if (sjcl.bitArray.bitLength(c) !== 128) throw new sjcl.exception.invalid("cbc iv must be 128 bits");
  var e = sjcl.bitArray, f = e.k, g = e.bitLength(b), h = 0, i = [];
  ...
  for (d = 0; h + 128 <= g; d += 4, h += 128) {
    ...
    i.splice(d, 0, c[0], c[1], c[2], c[3]);
  }
  g = (16 - (g >> 3 & 15)) * 16843009;
  ...
  g = h[d - 1] & 255;
  if (g == 0 || g > 16) throw new sjcl.exception.corrupt("pkcs#5 padding corrupt");
  c = g * 16843009;
  if (!e.equal(e.bitSlice([c, c, c, c], 0, g * 8), e.bitSlice(h, h.length * 32 - g * 8, h.length * 32))) throw new sjcl.exception.corrupt("pkcs#5 padding corrupt");
  return e.bitSlice(h, 0, h.length * 32 - g * 8);
}};
